...
async def get_agent_response(
    ...
    try:
        ...
        # 2. 构建聊天历史
        #LangChain 的 Agent 和 LLM 模型需要特定格式的消息对象才能正确处理对话上下文。
        # 原始的元组格式 [(str, str), ...] 无法被 LangChain 直接理解，因此需要转换成标准的消息对象格式。
        ...
        # 3. 流式执行
        full_response = [] # 用于收集 Agent 的最终回答内容（可能是分块返回的）
        steps = [] # 用于记录 Agent 的思考过程和工具调用步骤
        ...
        async for chunk in agent_executor.astream({
            ...
            if "output" in chunk: # 如果当前 chunk 包含 "output" 键，说明这是 Agent 的最终回答内容
                # 将回答内容追加到 full_response 列表中
                full_response.append(chunk["output"])
            elif "intermediate_steps" in chunk: # 中间步骤
...
async def get_agent_stream_response(
    ...
    thinking_queue = asyncio.Queue() # 异步队列: 用于在后台任务和主协程之间传递思考事件
    ...
    try:
        ...
        while not agent_done.is_set():
            try:
                # 使用短超时轮询队列，实现实时推送
                event = await asyncio.wait_for(thinking_queue.get(), timeout=0.1) # 设置超时,防止阻塞
                yield f"data: {json.dumps(event, ensure_ascii=False)}\n\n" # 通过yield发给前端
                thinking_queue.task_done() # 通知队列事件已被处理
            except TimeoutError:
                # 超时是正常的，继续等待
                continue
        ...
        while not thinking_queue.empty():
            try:
                event = thinking_queue.get_nowait() #非阻塞的取出剩余事件
                yield f"data: {json.dumps(event, ensure_ascii=False)}\n\n"
